...
AEROSPACE_MONITOR_COUNT=$(aerospace list-monitors | wc -l)
...
if [ "$SENDER" = "aerospace_workspace_change" ]; then
  ...
  for i in $AEROSAPCE_WORKSPACE_FOCUSED_MONITOR; do
	  sketchybar --set space.$i display=$((1 + $AEROSPACE_MONITOR_COUNT - $AEROSPACE_FOCUSED_MONITOR))
  done
  ...
  sketchybar --set space.$AEROSPACE_FOCUSED_WORKSPACE display=$AEROSPACE_WORKSPACE_FOCUSED_MONITOR
